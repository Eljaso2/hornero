...
### Núcleo 9 — Cómo Somos (antes Núcleo 6)

**Datos duros de la clase trabajadora con categorías del campo obrero (Iñigo Carrera).** Define la **taxonomía soberana** que estructura todo el ecosistema: es su epistemología. Presentación clara para dirigentes sindicales: cuántos somos, cómo estamos, hacia dónde vamos.

**Función central:** Cómo Somos define categorías → Nuestro Derecho (Núcleo 7) organiza datos con esas categorías → Laboratorio (Núcleo 2) produce modelos con esos datos → los demás núcleos usan esos modelos. La taxonomía soberana es la epistemología de todo el sistema.

**Fuentes:** INDEC, Ministerio de Trabajo, UCA (pobreza), Mate Economía, CTAA/IEFCTAA, PIMSA/Iñigo Carrera, Luis Campos.
...
- **Informes de Índices:** actualizaciones de ICE (N11), IFT (N13), Cómo Somos (N9).
...
**Volantes, comunicados, discursos de líderes, resoluciones de asambleas y congresos, presentaciones judiciales.** Lo que el sindicato **produce y decide** — la voz del campo obrero. No lo que protege (Nuestro Derecho, N7), sino lo que hace.

**Barra de conflictos abiertos:** muestra en tiempo real cuántos sectores están en conflicto abierto en el país, con nombre de sector y tipo de conflicto/empresa. Se actualiza con Reportes Gremiales (IS, N6). Registro de apertura y cierre de conflictos.

**Distinción:** Nuestro Derecho (N7) = lo que **protege** (leyes, convenios, fallos). Acción Sindical (N14) = lo que el sindicato **hace** (volantes, comunicados, discursos, resoluciones, presentaciones).
...
| 9 (Cómo Somos) | 6 | `hornero-capa3/nucleo9-como-somos/` |
...
| 14 (Acción Sindical) | — | `hornero-capa3/nucleo14-accion-sindical/` |
| 15 (SMVM) | — | `hornero-capa3/nucleo15-smvm/` |

---

### Núcleo 15 — Salario Mínimo Vital y Móvil (SMVM)

**El SMVM en contexto — no el número isolation sino el piso de vida.** Interfaz visible donde el trabajador entra a entender, comparar, y ver la distribución del ingreso. Los demás núcleos siguen produciendo sus índices (ICE×SMVM, IFT×SMVM) — N15 centraliza, presenta y permite diálogo.

5 componentes:

1. **Qué es el SMVM y cómo se mide:** concepto (LCT Art. 116-117), cómo lo miden los aceiteros (informes VE: salario real vs. SMVM, horas para SMVM, tercerización vs. SMVM del convenio) y cómo lo mide Mate (SMVM vs. canasta obrera, vs. salario medio, vs. inflación obrera). Comparación con otras mediciones como referencia.
2. **Ingresos por sector y rama:** tabla/visualización con ingresos por sector y fracción de clase (planta permanente, contratado, tercerizado, informal, estatal) vs. SMVM y canasta. Categorías del campo, no categorías INDEC.
3. **Chat comparativo de salario:** diálogo tipo CE donde el trabajador compara su salario contra SMVM, convenio, canasta, sector, distribución. Permite profundizar, conectar con ICE×SMVM (N11) e IFT×SMVM (N13).
4. **Distribución del ingreso nacional:** gráfico pirámide invertida con estratos Iñigo Carrera (alta burguesía → pauperización), ingresos promedio por estrato, % del ingreso nacional, posición del trabajador. Datos Mate + PIMSA, no deciles INDEC.
5. **Inflación real vs. oficial:** serie temporal mensual, inflación INDEC vs. inflación obrera real (Mate), SMVM nominal vs. SMVM real. Fuentes explicitadas en cada punto. Actualiza con cada Mirador MATE.

**Articulación:** N15 consume datos de N9 (Cómo Somos), N10 (Mirador MATE), N11 (ICE×SMVM), N13 (IFT×SMVM), N7 (convenios). Routea datos SMVM/canasta/inflación a N11 y N13. No produce datos primarios — centraliza, presenta, permite diálogo.

→ `hornero-capa3/nucleo15-smvm/nucleo15-smvm.md`
