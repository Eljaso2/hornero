...
class HorneroApp extends HoComponent {
  ...
  _styles() {
    return css`
      ...
      .chat-persona-img.investigador-scale { transform: scale(1.15); }
  ...
  _render() {
    ...
    if (this.screen === 'home') {
      ...
      const personaSlides = personas.map(p =>
        ...
        `<img src="${p.img}" alt="${p.alt}" class="chat-persona-img${p.periodistaFull ? ' periodista-full' : ''}${p.persona === 'sociologo' ? ' investigador-scale' : ''}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span class="persona-choice-emoji" style="display:none;font-size:4rem">${p.emoji}</span>` +
