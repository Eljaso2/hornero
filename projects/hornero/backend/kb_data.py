...
Jasinski retoma esta distinción en el Cap. 6 'La violencia empresarial' (pp. 157-171), donde desarrolla las formas concretas que tomó la violencia empresarial de La Forestal. Ver chunk 'kb-jasinski-formas-violencia' para la enumeración completa de las cinco formas.""",
...

    {
        ...
        "text": """LAS FORMAS DE LA VIOLENCIA EMPRESARIAL — Jasinski, El encanto del tanino, Cap. 1 (2023)

En el Capítulo 1, Jasinski plantea que la violencia empresarial es un fenómeno complejo que existe sin solución de continuidad desde los orígenes del capitalismo, y propone un esquema de cuatro formas para identificarla y caracterizarla. Partiendo de reconocer la posición de poder privilegiada del capital frente a las clases trabajadoras (el fundamento de la propiedad), distingue:

1. VIOLENCIA DEL TRABAJO — Marcelo Almeida propuso considerar las afecciones en la salud mental y física obrera como efecto necesario de la organización de la producción, la división del trabajo, el control externo del ritmo de producción, los abusos de contrato, la extensión de la jornada legal y las condiciones de trabajo. Aquí se identifican los accidentes y enfermedades laborales. (Jasinski, Cap. 1, p. 42)

2. VIOLENCIA REPRESIVA — Los daños físicos o psicológicos producto del autoritarismo dado por la relación jerárquica, agresiones explícitas o amenazas, castigos o reprimendas y humillaciones, el espionaje laboral y el sistema de gestión de la seguridad privada o policía empresarial. Incluye las "listas negras" o "boicot empresarial", entrega de información de los trabajadores a las fuerzas represivas, vinculación entre secuestro, desaparición o detención y despido, financiación de la represión o formación de policías privadas. (Jasinski, Cap. 1, pp. 42-43)

3. NORMALIZACIÓN DE LA VIOLENCIA — Un proceso que lleva a interiorizar el miedo al despido, a las suspensiones y rotaciones, y a la deslocalización productiva. Implica el control psicológico por la discrecionalidad en la distribución de recompensas, el trazado de fronteras sociales y laborales, procesos de etiquetamiento y estigmatización, y la instalación de un pensamiento unidimensional y de tolerancia o negación de represión contra el pensamiento disidente. (Jasinski, Cap. 1, p. 43)

4. VIOLENCIA SILENTE — Recuperando prácticas identificadas por Sutherland en la década de 1940 y que Wolfgang Naucke conceptualizó como "delito económico-político". Fundadas en el poder económico y motivadas por la ganancia, se agrupan prácticas reñidas con el principio de legalidad vigente, atentando contra las personas, las instituciones y el medio social. (Jasinski, Cap. 1, p. 43)

Marco teórico: Jasinski arranca con la tipología de Galtung (violencia directa, estructural, cultural) y la crítica de que la literatura especializada ha tendido a dar por finalizado el ciclo de las violencias en el control laboral. Desde el marxismo, el problema se inserta en la ficción de la separación de los momentos económicos y políticos: la creencia de que las "leyes naturales" de la producción y el carácter "libre" del trabajo anulan todo despotismo empresarial.

Iñigo Carrera, en el Prólogo (pp. 15-22), complementa con la distinción entre coacción extraeconómica (la que crea las condiciones del régimen) y coacción económica (la que lo perpetúa). Jasinski aplica estas cuatro formas a lo largo del libro, especialmente en el Cap. 6 'La violencia empresarial' (pp. 157-178), donde las desarrolla en el caso concreto de La Forestal: lockout como coacción económica por excelencia, destrucción de organizaciones, sometimiento por control del espacio, y depuración de disidentes.""",
...
def rebuild_knowledge_base_string() -> str:
    ...
    for chunk in source_chunks:
        ...
        if chunk_sources:
            lines.append(f"Fuente: {', '.join(chunk_sources)}")
        elif chunk.get('book_ref'):
            lines.append(f"Fuente: {chunk['book_ref']}")
        lines.append("")
...
def get_chunks_text(chunk_ids: list, max_chars_per_chunk: int = 6000) -> str:
    ...
    for chunk in chunks:
        ...
        chunk_sources = chunk.get('sources', [])
        if chunk_sources:
            lines.append(f"Referencia: {', '.join(chunk_sources)}")
        elif chunk.get('book_ref'):
            lines.append(f"Referencia: {chunk['book_ref']}")
